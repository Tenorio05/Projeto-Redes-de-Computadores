...
# modelo: id:[nomedo arquivo, valor, nome do ultimo a dar lance valido]
arquivos = {}
id_atual = 0

#lista de Compradores participando do leilão
# modelo: ip:[nome, estado de prontidão]
Compradores = {}

#status de operação do leilao
Operation = "WAIT"
num_lances = 0
tempo_leilao = 0.0
...
def Ready(endereco, udp):
    if endereco not in Compradores:
        msg = {"tipo": "text", 
               "msg": "Vc não está cadastrado, não pode executar essa funcao"}
    else:
...
def Lance(id, valor, endereco, udp):
    ...
    if endereco not in Compradores:
        ...
        msg = {"tipo": "text", 
               "msg": "Este lance n foi valido, este não é o id do item em leilao no momento"}
        num = 0

    elif arquivos[id][1] >= valor:
        msg = {"tipo": "text", 
               "msg": "Este lance n foi valido, o valor levantado nao superou o valor atual do item"}
        num = 0
...
def Status(endereco, udp):
    if endereco not in Compradores:
        ...
        msg = {"tipo": "text&dado", 
               "msg": "Este é o comprador que esta ganhando no leilao atual",
        ...
        if Compradores[i][0] == arquivos[id_atual][2]:
            with open(arquivos[id_atual][0], 'rb') as f:
                while True:
                    ...
                    msg = {
                        "tipo": "text&arquivo",
                        "msg": f"Parabens! Você conseguiu o {arquivos[id_atual][0]}!",
            ...
            msg = {"msg": "Mais sorte na próxima! Enviando o arquivo para o ganhador, aguarde..."} 
            SendTo(msg, i, udp)
